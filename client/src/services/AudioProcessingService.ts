...
export interface EmotionMetadata {
  ...
  intensity: number;        // 0-1 scale of emotion intensity
  variability: number;      // 0-1 scale of emotion variation
...
export interface BaseAudioMetadata {
  ...
  format: {
    container: string;     // e.g., 'wav', 'mp3', 'webm'
    codec: string;         // e.g., 'pcm_f32le', 'opus'
  ...
  duration_ms: number;    // Precise duration in milliseconds
  size_bytes: number;     // File size in bytes
  transcription: {
    text: string;         // Full text transcription
...
export interface AudioFile extends BaseAudioMetadata {
  processingProgress?: number;  // Progress percentage for processing
  wordCount?: number;          // Number of words detected
  averageWordConfidence?: number; // Average confidence across all words
  noiseLevel?: number;         // Detected background noise level
  segments?: {                 // Time segments for each detected word
    start: number;
    end: number;
    word: string;
    confidence: number;
...
export interface WordFile extends BaseAudioMetadata {
  originalAudioId: string;     // Reference to source audio
  word: string;                // The actual word
  confidence: number;          // Word detection confidence
  phonemes: {                  // Phonetic breakdown
    phoneme: string;
    start: number;
    end: number;
    confidence: number;
  }[];
  position: {                  // Position in original audio
    start: number;
    end: number;
    wordIndex: number;         // Index in sentence
  };
  ...
  acousticFeatures: {          // Acoustic characteristics
    pitch: number[];           // Pitch contour
    intensity: number[];       // Intensity contour
    formants: number[][];      // Formant frequencies
    voicing: boolean[];        // Voicing detection
  };
  classification: {            // Word classifications
    partOfSpeech: string;      // e.g., noun, verb, adjective
    isStopWord: boolean;       // Whether it's a common stop word
    sentiment?: number;        // Sentiment score if applicable
    isFiller: boolean;         // Whether it's a filler word (um, uh, etc.)
  };
}
