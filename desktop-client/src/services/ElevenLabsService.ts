...
export class ElevenLabsService {
  ...
  /**
   * Get current location for metadata (Desktop version)
   */
  ...
  /**
   * Save audio file locally (Desktop version using Electron APIs)
   */
  async saveAudioLocally(audioBlob: Blob, filename: string): Promise<string> {
    try {
      // In Electron, we can use the main process to save files
      const arrayBuffer = await audioBlob.arrayBuffer();
      const buffer = Buffer.from(arrayBuffer);
      
      // Send to main process via IPC
      if (window.electronAPI) {
        const savedPath = await window.electronAPI.saveAudioFile(buffer, filename);
        return savedPath;
      } else {
